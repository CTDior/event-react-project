...
const Result = ({ event }: Props) => {
  ...
  const { favorites, addFavorite, removeFavorite } =
    useContext(FavoritesContext); // this gets the add favorite function from context
  ...
  return (
    <li className="Result">
      ...
      <p>{venueName}</p>
      <a className="tmlink" href={event.url}>
        Get Your Tickets Here!
      </a>
      <p>
        {isFav(event.url) === false && (
          <button className="favorite" onClick={() => addFavorite(event)}>
            Add to Bucket List &hearts;
          </button>
        )}
        {isFav(event.url) && (
          <button
            className="favorite"
            onClick={() => removeFavorite(event.url)}
          >
